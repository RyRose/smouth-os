...
int DetectMemory(MemoryRegion *regions, int len) {
  ...
  for (i = 0; i < count && i < len; i++) {
    ...
    region.type = static_cast<MemoryRegionType>(
        mmap_entries[i].type); // hack(ryanthrose) handle errors on type
    regions[i] = region;
  }
  return i;
}

void *GetKernelStart() { return &kernel_start; }

void *GetKernelEnd() { return &kernel_end; }
