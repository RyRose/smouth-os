//! 16550A UART serial support for the x86 QEMU target.
...
pub fn init() void {
    ...
    // Disable interrupts
    ioport.outb(.com1_ier, 0x00);

    // Enable DLAB
    ioport.outb(.com1_lcr, 0x80);

    // Set baud rate divisor to 3 (38400)
    ioport.outb(.com1_data, 0x03); // low byte
    ioport.outb(.com1_ier, 0x00); // high byte

    // 8 bits, no parity, one stop bit
    ioport.outb(.com1_lcr, 0x03);

    // Enable FIFO, clear, 14-byte threshold
    ioport.outb(.com1_iir_fcr, 0xC7);

    // IRQs enabled, RTS/DSR set
    ioport.outb(.com1_mcr, 0x0B);
}

/// Check if the transmit buffer is empty.
fn isTransmitEmpty() bool {
    return (ioport.inb(.com1_lsr) & 0x20) == 0;
}

/// Write a byte to the serial port.
fn writeByte(b: u8) void {
    while (isTransmitEmpty()) {}
    ioport.outb(.com1_data, b);
}
