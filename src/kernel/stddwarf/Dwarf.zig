...
pub const Die = struct {
    ...
    pub fn getAttrString(
        ...
        switch (form_value.*) {
            ...
            .strx => |index| {
                const debug_str_section = di.section(.debug_str_offsets) orelse return bad();
                const debug_str_offsets = debug_str_section.data;
                ...
                switch (compile_unit.format) {
                    .@"32" => {
                        ...
                        return getStringGeneric(opt, offset);
                    },
                    .@"64" => {
...
pub fn section(di: Dwarf, dwarf_section: Section.Id) ?Section {
    return if (di.sections[@intFromEnum(dwarf_section)]) |s| s else null;
}
...
fn scanAllFunctions(di: *Dwarf, allocator: Allocator) ScanError!void {
    var fbr: FixedBufferReader = .{ .buf = di.section(.debug_info).?.data, .endian = di.endian };
                    ...
                    const fn_name = x: {
                        ...
                        for (0..3) |_| {
                            if (this_die_obj.getAttr(AT.name)) |_| {
                                break :x try this_die_obj.getAttrString(
                                    di,
                                    AT.name,
...
const DebugRangeIterator = struct {
    ...
    pub fn init(ranges_value: *const FormValue, di: *const Dwarf, compile_unit: *const CompileUnit) !@This() {
        ...
        const debug_section = di.section(section_type) orelse return error.MissingDebugInfo;
        const debug_ranges = debug_section.data;
...
fn parseAbbrevTable(di: *Dwarf, allocator: Allocator, offset: u64) !Abbrev.Table {
    var fbr: FixedBufferReader = .{
        .buf = di.section(.debug_abbrev).?.data,
...
fn readDebugAddr(di: Dwarf, compile_unit: CompileUnit, index: u64) !u64 {
    const debug_section = di.section(.debug_addr) orelse return bad();
    const debug_addr = debug_section.data;
...
pub fn scanAllUnwindInfo(di: *Dwarf, allocator: Allocator, base_address: usize) !void {
    if (di.section(.eh_frame_hdr)) |eh_frame_hdr_section| blk: {
        const eh_frame_hdr = eh_frame_hdr_section.data;
...
pub fn scanCieFdeInfo(di: *Dwarf, allocator: Allocator, base_address: usize) !void {
    ...
    for (frame_sections) |frame_section| {
        if (di.section(frame_section)) |s| {
            const section_data = s.data;
...
fn getStringGeneric(opt: ?Section, offset: u64) ![:0]const u8 {
    const s = opt orelse return bad();
    const str = s.data;
    ...
    if (di.findCompileUnit(address)) |compile_unit| {
        return .{
            ...
            .compile_unit_name = compile_unit.die.getAttrString(
                di,
                std.dwarf.AT.name,
                if (di.section(.debug_str)) |s| s.data else null,
                compile_unit.*,
            ) catch |err| switch (err) {
